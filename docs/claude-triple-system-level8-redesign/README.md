...
1. 默认轻量，不把复杂任务的治理成本强加给所有任务。
2. 自动化优先覆盖“高收益、低风险、低噪音”的动作。
3. 高价值但高成本的流程按条件触发，而不是默认触发。
4. 用户指令始终高于系统流程。
5. 流程必须可以降级，不能因为一个子系统失效就阻塞所有工作。
6. 记忆只保留决策、约束和未完成事项，不保留流水账。

## 推荐实施顺序

1. 先落地 `routing.md` 的模式分流。
2. 再按照 `automation.md` 裁剪 hooks。
3. 再按 `permissions.md` 收紧默认权限。
