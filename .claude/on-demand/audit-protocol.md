...
> 
> 三层体系详见 `SYSTEM.md` §13 和 `scripts/audit/`（2026-09-13 起根级单一目录）。
>
...
Layer 2: scripts/audit/run-all.sh          手动 + /verify 流程调用：13 项静态检查 + 条件 bonus（数量以 run-all.sh 输出为准）（统一入口，skipped≠pass）
