...
next: 继续实施
heavy_deps: sprint-memory〔已退役归档 2026-08-02 B7；sprint 文件为历史遗存，if-exists 读取〕（Heavy 模式下读取冲刺记忆）
---
...
### Step 1b: Load sprint memory (mid-term context)

Check for the current sprint memory file:
1. Calculate current ISO week: `sprint-YYYY-WNN.md`
2. Look in `.claude/memory/` for matching file
3. If found, read it — this provides cross-session context (decisions, lessons, unfinished work)
4. Also check `.claude/shared-state/board.json` for active multi-agent workflow state
...
SPRINT CONTEXT (mid-term memory):
[If sprint file exists: key decisions and lessons from this sprint]
[If not: "No sprint memory found for current week"]

SHARED STATE (multi-agent):
[If board.json has active tasks: show task summary and agent status]
[If empty: "No active multi-agent workflow"]
