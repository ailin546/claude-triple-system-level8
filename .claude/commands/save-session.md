...
## Sprint Memory Snapshot

[Auto-populated from .claude/memory/sprint-YYYY-WNN.md if it exists.
Include key decisions and lessons from the current sprint that provide
context beyond this single session.]

---

## Shared State Snapshot

[Auto-populated from .claude/shared-state/board.json if active tasks exist.
Shows in-flight multi-agent workflow state so the next session can resume
orchestration.]
```
