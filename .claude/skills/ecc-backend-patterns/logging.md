...
Emit JSON log entries for machine-readable observability:
...
**Best practices**:
- Always include a `requestId` for tracing across services
- Use `warn` for expected degraded states, `error` for unexpected failures
- Never log PII (emails, tokens, passwords) — log IDs instead
