...
- [2026-08-17] `openai-codex` 插件僵尸作业锁死整条 Codex 通道 9 小时（表现：新任务 0 秒 failed，外部完全看不出原因）→ 根因：作业终态**只由作业自己的进程写**，`state.mjs` 存了 `pid` 但没有任何地方去看它；进程被外部杀掉（ENOSPC / 会话结束 / SIGKILL）后记录永远停在 `running`，`codex-companion.mjs::resolveLatestTrackedTaskThread` 见到「上一个还在跑」就拒绝所有后续提交。**本地补丁**：`lib/state.mjs` 的 `listJobs()` 加 `reconcileOrphanedJobs()` —— 活性判据来自 OS 不来自记录（`process.kill(pid,0)`，EPERM 算活），并用 `ps -o lstart=` 挡 pid 复用（进程启动早于作业记录 60s 以上 = 别人的 pid）。**纯投影不写回**：磁盘状态仍是权威，因此幂等、无副作用、不与真正的写者打架。三类不可观测输入一律保持原状（无 pid / `ps` 读不出 / 解析不出时间），**绝不用计时器代替观测**。6 分支测试（活 pid 留 running / 死 pid → failed / pid 复用 → failed / 无 pid 留 running / 已完成不动 / queued 同样收割）。**元教训：这正是 CCHFT 不变量 #79+#80 的镜像反例——「还没完成」的状态只能被死掉的当事人自己解除、而没有任何观测者，等价于永不解除；任何"活跃/在飞/未完成"状态机都必须配一个外部观测通道，否则它的失效模式不是报错而是静默锁死。** ⚠ 补丁在 `~/.claude/plugins/cache/openai-codex/codex/1.0.4/`（插件缓存，升级会被覆盖），已埋 grep 锚 `CCHFT-LOCAL-PATCH-ORPHAN-REAP`；Codex 通道再次出现「0 秒 failed / 说上一个还在跑」时，先 grep 该锚确认补丁是否还在
